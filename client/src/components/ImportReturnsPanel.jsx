...
import { parseReturnsWorkbook } from '../utils/parseSalesFile.js';
import { formatCurrency } from '../utils/currency.js';

function downloadTemplate() {
  const csv = 'Date,Amount,Notes\n2026-01-15,5000.00,Example row - delete me\n';
  const blob = new Blob([csv], { type: 'text/csv' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = 'returns-import-template.csv';
  a.click();
  URL.revokeObjectURL(url);
}

export default function ImportReturnsPanel({ companyId, currency, onImported }) {
  ...
  function handleFileChange(e) {
    ...
    reader.onload = (evt) => {
      try {
        const result = parseReturnsWorkbook(evt.target.result);
        setParsed(result);
      } catch (err) {
  ...
  async function handleImport() {
    ...
    setImportError(null);
    try {
      const { data } = await api.post(`/companies/${companyId}/returns/bulk`, { returns: parsed.rows });
  ...
  return (
    <div className="card card-pad" style={{ marginBottom: '1.5rem', background: 'var(--paper-dim)' }}>
      ...
      >
        <span>Import Returns from a Spreadsheet</span>
        <button
          type="button"
